...
static struct clk_vdd_class *cam_cc_yupik_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_regmap *cam_cc_yupik_clocks[] = {
...
static const struct regmap_config cam_cc_yupik_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
	.max_register = 0xf00c,
...
static struct qcom_cc_desc cam_cc_yupik_desc = {
	...
	.clk_regulators = cam_cc_yupik_regulators,
	.num_clk_regulators = ARRAY_SIZE(cam_cc_yupik_regulators),
};

static const struct of_device_id cam_cc_yupik_match_table[] = {
	{ .compatible = "qcom,yupik-camcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, cam_cc_yupik_match_table);

static int cam_cc_yupik_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &cam_cc_yupik_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &cam_cc_yupik_desc, regmap);
	if (ret) {
...
{
	qcom_cc_sync_state(dev, &cam_cc_yupik_desc);
}

static struct platform_driver cam_cc_yupik_driver = {
	.probe = cam_cc_yupik_probe,
	.driver = {
		.name = "cam_cc-yupik",
...
static int __init cam_cc_yupik_init(void)
{
	return platform_driver_register(&cam_cc_yupik_driver);
}
subsys_initcall(cam_cc_yupik_init);

static void __exit cam_cc_yupik_exit(void)
{
	platform_driver_unregister(&cam_cc_yupik_driver);
}
module_exit(cam_cc_yupik_exit);

MODULE_DESCRIPTION("QTI CAM_CC YUPIK Driver");
