...
#define EMAC_PINCTRL_STATE_MDIO_ACTIVE_DATA "emac_mdio_active_data"
	...
	if (phy->external) {
		if (mdio) {
			result = pinctrl_select_state(adpt->pinctrl,
						      adpt->mdio_pins_active_clk);
			if (result)
				emac_err(adpt,
					 "error:%d Can not switch on %s pins\n",
					 result,
					 EMAC_PINCTRL_STATE_MDIO_ACTIVE_CLK);
			ret = result;

			result = pinctrl_select_state(adpt->pinctrl,
						      adpt->mdio_pins_active_data);
			if (result)
				emac_err(adpt,
					 "error:%d Can not switch on %s pins\n",
					 result,
					 EMAC_PINCTRL_STATE_MDIO_ACTIVE_DATA);
			ret = result;
		}
...
static int emac_open(struct net_device *netdev)
{
	...
	if (netdev->phydev) {
		phy_attached_info(netdev->phydev);
	}
	return retval;
...
static int msm_emac_pinctrl_init(struct emac_adapter *adpt, struct device *dev)
{
	...
	adpt->mdio_pins_active_clk = pinctrl_lookup_state(adpt->pinctrl,
				EMAC_PINCTRL_STATE_MDIO_ACTIVE_CLK);
	if (IS_ERR_OR_NULL(adpt->mdio_pins_active_clk)) {
		emac_dbg(adpt, probe, "error:%ld Failed to lookup mdio clk pinctrl active state\n",
			 PTR_ERR(adpt->mdio_pins_active_clk));
		return PTR_ERR(adpt->mdio_pins_active_clk);
	}
	...
	adpt->mdio_pins_active_data = pinctrl_lookup_state(adpt->pinctrl,
				EMAC_PINCTRL_STATE_MDIO_ACTIVE_DATA);
	if (IS_ERR_OR_NULL(adpt->mdio_pins_active_data)) {
		emac_dbg(adpt, probe, "error:%ld Failed to lookup mdio data pinctrl active state\n",
			 PTR_ERR(adpt->mdio_pins_active_data));
		return PTR_ERR(adpt->mdio_pins_active_data);
	}
		...
		size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < 32; i++) {
		phydata = adpt->mii_bus->read(adpt->mii_bus,  adpt->phydev->mdio.addr, i);
		len += scnprintf(buf + len, buf_len - len,
				"MII Register (%#x) = %#x\n",
				i, phydata);
		                ...
		                size_t count, loff_t *ppos)
{
	...
	if (page == 1) {
		/* switch back to copper page */
		phy_write(adpt->phydev, REG_CHIP_CONFIG, ccr | BT_BX_REG_SEL);
		phydata = adpt->mii_bus->read(adpt->mii_bus,  adpt->phydev->mdio.addr, regnum);
		return count;
	 } else if (page == 2) {
		/* switch to SGMII/fiber page */
		 phy_write(adpt->phydev, REG_CHIP_CONFIG, ccr & ~BT_BX_REG_SEL);
	...
	if (val >= 0 && regnum >= 0) {
		...
		phydata = adpt->mii_bus->write(adpt->mii_bus,  adpt->phydev->mdio.addr, regnum, val);
